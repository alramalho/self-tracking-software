import React from "react";

interface MetricBarChartProps {
  data: number[];
  color?:
    | "yellow"
    | "blue"
    | "green"
    | "rose"
    | "pink"
    | "red"
    | "orange"
    | "amber"
    | "purple"
    | "gray";
  className?: string;
  dimmed?: boolean; // For comparison mode (last week)
  label?: string; // Optional label like "This week" or "Last week"
}

const getColorsFromName = (colorName: string): { bg: string; text: string } => {
  const colorMap: { [key: string]: { bg: string; text: string } } = {
    blue: { bg: "bg-blue-200", text: "text-blue-700" },
    yellow: { bg: "bg-yellow-200", text: "text-yellow-700" },
    green: { bg: "bg-green-200", text: "text-green-700" },
    rose: { bg: "bg-rose-200", text: "text-rose-700" },
    pink: { bg: "bg-pink-200", text: "text-pink-700" },
    red: { bg: "bg-red-200", text: "text-red-700" },
    orange: { bg: "bg-orange-200", text: "text-orange-700" },
    amber: { bg: "bg-amber-200", text: "text-amber-700" },
    purple: { bg: "bg-purple-200", text: "text-purple-700" },
    gray: { bg: "bg-gray-200", text: "text-gray-700" },
  };

  return colorMap[colorName] || { bg: "bg-gray-200", text: "text-gray-700" };
};

export const MetricBarChart: React.FC<MetricBarChartProps> = ({
  data,
  color = "green",
  className = "flex gap-1 items-end",
  dimmed = false,
  label,
}) => {
  const { bg, text } = getColorsFromName(color);

  return (
    <div className="space-y-1">
      {label && (
        <div className={`text-xs font-medium ${dimmed ? "text-muted-foreground/60" : "text-muted-foreground"}`}>
          {label}
        </div>
      )}
      <div className={className}>
        {data.map((value, i) => {
          if (value === 0) {
            // Special case for 0: show N/A or empty state
            return (
              <div key={i} className="flex-1 relative">
                <div className="relative h-2 mt-6">
                  <span
                    className={`absolute -top-4 left-1/2 transform -translate-x-1/2 text-xs font-medium ${text} opacity-30`}
                  >
                    —
                  </span>
                </div>
                <div
                  className={`${bg} ${dimmed ? "opacity-30" : "opacity-20"} rounded-sm relative`}
                  style={{ height: `1px` }}
                />
              </div>
            );
          }

          // Normal case: show bar with number centered
          const height = value * 8;
          return (
            <div key={i} className="flex-1 relative">
              <div
                className={`${bg} ${dimmed ? "opacity-40" : ""} rounded-sm relative transition-all duration-200`}
                style={{ height: `${height}px` }}
              >
                <span
                  className={`absolute inset-0 flex items-center justify-center text-xs font-medium ${text} ${dimmed ? "opacity-60" : ""}`}
                >
                  {value}
                </span>
              </div>
            </div>
          );
        })}
      </div>
    </div>
  );
};
