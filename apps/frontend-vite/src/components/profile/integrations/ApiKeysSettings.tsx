...
const DOCS_URL = "https://tracking.so/docs/bring-your-own-curriculum";
...
4. If I have no plans yet, interview me briefly (goal, an emoji, frequency target or dated sessions, finishing date, which loggable activities) and create my first plan with \`create_plan\`. Then ask whether I have a self-built curriculum (markdown folder) to attach with \`replace_curriculum\`.`
  ...
  return (
    <div className="space-y-6">
        ...
        {setupPrompt && mcpCommand ? (
          <>
            <p className="text-sm text-muted-foreground">
              ...
              the MCP server, installs the usage skill, verifies the connection,
              and onboards you if you have no plans yet. You can read every step
              before sending it.
            </p>
      ...
      {/* Step 3: what you get */}
      <div className="space-y-3">
        <p className="text-sm font-medium text-foreground">3. Use it</p>
        <p className="text-sm text-muted-foreground">
          ...
          to hand it week planning — the plan shows when it last synced, and
          your coach steps back in if it goes quiet. Try: {'"'}push my
      ...
      <div className="space-y-2">
        ...
        {isLoading ? (
          ...
          <p className="p-3 text-sm text-muted-foreground">No API keys yet.</p>
        ) : (
          keys.map((key) => (
            <div
              key={key.id}
