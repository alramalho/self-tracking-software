...
import { AlertCircle, Goal, Check, Crown, Sparkles, Info } from "lucide-react";
...
function PlanGoalSetter() {
  ...
  // For paid users: track if they just upgraded and are waiting
  const [waitingForUpgrade, setWaitingForUpgrade] = useState(false);

  // Poll for upgrade status when waiting
  useEffect(() => {
  ...
  // When user becomes premium while waiting, show the paid user popover to let them select
  useEffect(() => {
    if (waitingForUpgrade && isUserPremium && extractedPlans.length > 1) {
      // User just upgraded - show the popover again so they can select which goal to create
  ...
  const handleSubmit = async (
    ...
    try {
      ...
      // Store extracted plans and show popover immediately if multiple goals detected
  ...
  const renderExtractedData = (data: PlanGoalSetterResponse) => {
    ...
    // Multiple plans detected - show summary
    return (
      <div className="space-y-4">
  ...
  const handleAccept = async (data: PlanGoalSetterResponse): Promise<void> => {
    ...
    // Multiple plans detected - show selection popover
    setShowMultiplePlansPopover(true);
  };
  ...
  const handleProceedWithSelectedGoal = () => {
    ...
    // Save non-selected goals for later
    ...
    setShowMultiplePlansPopover(false);
    toast.success("You'll be able to create the other plan from your homepage after onboarding");
  };
      ...
      >
        <div className="space-y-6 pt-6 pb-4">
          <div className="flex flex-col items-center text-center">
            ...
            <p className="text-sm text-muted-foreground">
              Let's set up one plan now. You'll be able to create the other from your homepage after onboarding.
            </p>
          </div>
