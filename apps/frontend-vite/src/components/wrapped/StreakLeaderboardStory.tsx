...
import type { FriendScore } from "./FriendsLeaderboardStory";

interface StreakLeaderboardStoryProps {
  year: number;
  friends: FriendScore[];
  currentUser: FriendScore;
}

export const StreakLeaderboardStory: React.FC<StreakLeaderboardStoryProps> = ({
  year,
  friends,
  currentUser,
}) => {
  ...
  const ranked = useMemo(() => {
    const all = [currentUser, ...friends].filter((p) => p.bestStreak > 0);
    return all.sort((a, b) => b.bestStreak - a.bestStreak);
  }, [friends, currentUser]);
  ...
  return (
    ...
    >
      ...
      <div className={`absolute top-[5%] right-[-10%] w-[50%] h-[50%] rounded-full blur-3xl ${isLightMode ? "bg-red-200/30" : "bg-red-900/15"}`} />
            ...
            return (
              ...
              >
                ...
                <span
                  ...
                  style={{ color: isFirst ? (isLightMode ? "#ea580c" : "#fb923c") : level.color }}
                >
            ...
            return (
              <motion.div
                ...
                className={`flex items-center gap-3 px-4 py-2.5 rounded-xl ${
                  isYou
                    ? isLightMode ? "bg-orange-50/80 ring-1 ring-orange-200/50" : "bg-white/8 ring-1 ring-white/15"
              ...
              >
                ...
                <div className="flex items-center gap-1.5 text-sm text-right shrink-0">
                  ...
                  <span className={`font-mono font-semibold ${isLightMode ? "text-orange-600" : "text-orange-400"}`}>
                    {person.bestStreak}
                  </span>
...
export default StreakLeaderboardStory;
