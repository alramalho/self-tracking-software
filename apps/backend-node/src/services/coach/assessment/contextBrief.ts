...
import type {
  CoachContextBrief,
  CoachContextBriefCandidate,
  DifficultyPatternInsight,
  MetricsLoggingGapInsight,
  PlanMotivatorInsight,
  SelectedCoachInsight,
} from "./types";

type CoachPlan = Plan & { activities: Activity[] };

export async function buildCoachContextBrief(input: {
  user: User;
  plans: CoachPlan[];
  now: Date;
  lookbackDays?: number;
  ...
  return {
    planMotivators: plans.map((plan) => ({
      ...
      discardedReasons: extractDiscardedReasons(plan.coachNotes),
    })),
    difficultyPatterns: plans
      .map((plan) => {
        ...
        return buildDifficultyPattern(plan, planEntries);
      })
      .filter((pattern): pattern is DifficultyPatternInsight => pattern !== null),
    metricsLoggingGap: buildMetricsLoggingGap({
...
export function pickInsightForCandidate(input: {
  ...
  switch (candidate.type) {
    case "PLAN_ADJUSTMENT":
      return (
        formatDifficultyInsight(relevantDifficulty) ||
        formatMotivatorInsight(relevantMotivator)
      );
    case "SESSION_PREP":
    ...
    case "WEEK_RECAP":
      return (
        formatMetricsLoggingGapInsight(brief.metricsLoggingGap) ||
        formatDifficultyInsight(relevantDifficulty) ||
        formatMotivatorInsight(relevantMotivator)
      );
    case "CELEBRATION":
...
export function formatSelectedInsight(insight: SelectedCoachInsight): string {
  if (!insight) return "";
...
function buildDifficultyPattern(
  plan: CoachPlan,
...
function buildMetricsLoggingGap(input: {
  metricCount: number;
  ...
  const { metricCount, entriesInLookback, latestMetricCreatedAt, now, lookbackDays } =
    input;
...
function formatDifficultyInsight(
...
function formatMotivatorInsight(
  ...
  if (insight.selectedReason) {
    return {
      kind: "goal_reason",
      text: `The user chose "${insight.goal}" because they want to ${normalizeReason(insight.selectedReason)}. Use this as the motivational anchor only if it fits naturally.`,
    };
  }
...
function formatMetricsLoggingGapInsight(
...
function extractDiscardedReasons(coachNotes: string | null): string[] {
