...
export class SESService {
  ...
  constructor() {
    ...
    this.defaultFromEmail = process.env.SES_FROM_EMAIL || 'noreply@tracking.so';
  }
  ...
  async sendEmail(data: EmailData): Promise<string> {
    try {
      ...
      logger.info(`Successfully sent email to ${recipients.join(', ')} with MessageId: ${result.MessageId}`);
  ...
  async sendBulkEmail(emails: EmailData[]): Promise<string[]> {
    ...
    logger.info(`Sent ${emails.length} bulk emails, ${messageIds.filter(id => id !== 'failed').length} successful`);
    return messageIds;
  }
