...
function buildCoachAttentionSystemMessage(item: {
  title: string;
  message: string;
  ...
  return [
    ...
    "",
    "The user clicked \"Ask coach to fix\" from a plan warning generated by the app. This was not typed by the user.",
    "",
    "Attention item:",
    item.title,
    item.message,
    `Facts: ${facts}`,
    "",
    "Respond as the coach. Explain the schedule gap briefly and propose a concrete schedule repair or extension for the affected plan. Use the plan modification tool only if you can make a concrete proposal.",
  ].join("\n");
}
    ...
    try {
      ...
      if (!attentionItem) {
        return res.status(409).json({
          error: "This plan warning is no longer active",
        });
      }
      ...
      const systemContent = buildCoachAttentionSystemMessage(attentionItem);
