...
The current TestFlight distribution is build 160, locally built, validated, uploaded, processed and approved for internal and external beta testing. The private `Friends & Family` group has five email-invited testers; its public link is disabled. Recipients must still accept their invitations and install the app. The ignored App Store Connect API key used by the repeatable local workflow lives at `.release/appstore-connect/AuthKey_RRBPAL9WF4.p8`; never commit that file or its contents.
