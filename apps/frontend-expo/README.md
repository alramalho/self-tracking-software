...
The current TestFlight distribution is build 160, locally built, validated, uploaded, processed and approved for internal and external beta testing. The private `Friends & Family` group has one tester, `alexandre.ramalho.1998@gmail.com`; its public link is disabled and Apple accepted a fresh invitation request. The tester must still accept the invitation and install the app. The ignored App Store Connect API key used by the repeatable local workflow lives at `.release/appstore-connect/AuthKey_RRBPAL9WF4.p8`; never commit that file or its contents.
