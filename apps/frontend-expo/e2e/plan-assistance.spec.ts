...
for(const theme of ['DARK','LIGHT']) test(`assistance separates choices and preserves consent in ${theme}`,async({page,request})=>{
 ...
 await page.getByRole('button',{name:'Coaching',exact:true}).click();await page.getByRole('checkbox',{name:'Help me stay consistent',exact:true}).click();await page.getByRole('checkbox',{name:'Weekly review',exact:true}).check();await page.getByRole('button',{name:'Sun',exact:true}).click();await page.getByLabel('Time',{exact:true}).fill('19:00');await page.getByRole('button',{name:'Save coaching preferences',exact:true}).click();
 await page.reload();await expect(page.getByRole('button',{name:'Coaching',exact:true})).toContainText('Sun review · 19:00');
...
test('day reminders validate time and reviews respect entitlement',async({page,request})=>{
 await request.post(`${API}/__reset`);await request.patch(`${API}/users/user`,{headers,data:{planType:'FREE'}});await page.goto('/plan/fitness');const sheet=page.getByTestId('plan-assistance-sheet');
 await page.getByRole('button',{name:'Coaching',exact:true}).click();await expect(page.getByRole('checkbox',{name:'Help me stay consistent',exact:true})).toBeDisabled();await page.getByRole('button',{name:'Close',exact:true}).click();await page.goto('/plan-support/fitness');
