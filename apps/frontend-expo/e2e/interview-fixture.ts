...
export function interviewFixture(state: InterviewState, answer: string): InterviewResult {
  ...
  const needsImprovement =
    !rejected && state.stage === "goal" && !/because|express|matters|enjoy|love|feel/i.test(answer);
  const questions = {
  ...
  return { accepted: !rejected, needsImprovement, summary: rejected ? "That doesn’t give me a consistent, actionable answer yet. Let’s clarify it." : needsImprovement ? "The goal is clear. One useful detail is still optional." : "We’ll build around your goal and the time you actually have.", checks: [{ label: rejected ? "Needs clarification" : needsImprovement ? "Useful context" : "Concrete and consistent", required: !needsImprovement, passed: !rejected && !needsImprovement, detail: rejected ? "Please clarify your answer." : needsImprovement ? "Why it matters is helpful, but not required." : "This fits what you told me." }], facts, question, nextQuestion: question };
}
