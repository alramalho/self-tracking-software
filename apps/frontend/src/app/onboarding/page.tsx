...
const OnboardingStepRenderer = () => {
  ...
  return (
    <OnboardingContainer name={currentStepData?.id || "error"}>
      ...
      <ChevronLeft
        onClick={prevStep}
        className="fixed m-0 top-2 left-2 transform -translate-y-1/2 text-gray-500 hover:text-gray-700 cursor-pointer"
      />
      {isStepCompletedCallback && hasNextStep && (
        <ChevronRight
          onClick={nextStep}
          className="fixed m-0 top-2 right-2 transform -translate-y-1/2 text-gray-500 hover:text-gray-700 cursor-pointer"
        />
      )}
