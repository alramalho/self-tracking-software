...
router.post('/check-plan-goal', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { message, question_checks } = req.body;

    if (!message || !question_checks) {
      return res.status(400).json({ error: 'message and question_checks are required' });
    }
    ...
    const conversationHistory = await memoryService.readConversationHistory(req.user!.id, 30);
...
router.post('/generate-plan-activities', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const activitiesResult = await aiService.extractActivitiesForPlan(message, userContext);
...
router.post('/generate-plans', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const guidelinesResult = await extractGuidelinesAndEmoji(plan_goal, plan_progress);
...
async function extractGuidelinesAndEmoji(planGoal: string, planProgress: string): Promise<{
  guidelines: string;
  ...
  try {
    ...
    return {
      guidelines: 'Follow a progressive approach with consistency and gradual increases.',
      timeframes: [
