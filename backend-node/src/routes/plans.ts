...
router.get('/generate-invitation-link', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (!plan || plan.userId !== req.user!.id) {
      return res.status(403).json({ error: 'Not authorized to generate link for this plan' });
    }
...
// Create a new plan
router.post('/create-plan', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    // Create associated activities if provided
    if (planData.activityIds && planData.activityIds.length > 0) {
      await Promise.all(
        planData.activityIds.map((activityId: string) =>
    ...
    if (planData.sessions && planData.sessions.length > 0) {
      await Promise.all(
        planData.sessions.map((session: any) =>
          prisma.planSession.create({
            data: {
              planId: newPlan.id,
    ...
    // Create a plan group for the new plan
    ...
    // Update plan with group ID
    await prisma.plan.update({
      where: { id: newPlan.id },
      data: { planGroupId: planGroup.id },
    });

    // Update user to add this plan to their plan IDs (if planIds field exists in User model)
    // This would need to be implemented based on the actual User model structure

    logger.info(`Created plan ${newPlan.id} for user ${req.user!.id}`);
    res.status(201).json({
      plan: newPlan,
...
router.delete('/remove-plan/:planId', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
...
// Get user's plans
router.get('/user-plans', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
...
// Get specific plan
router.get('/plans/:planId', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { planId } = req.params;
...
router.post('/generate-sessions', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { goal, finishingDate, activities, description, existing_plan } = req.body;

    if (!goal || !activities) {
      return res.status(400).json({ error: 'goal and activities are required' });
    }
...
// Update plan
router.post('/plans/:planId/update', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { planId } = req.params;
    const { data } = req.body;
...
// Update plan order
router.post('/plans/update-plan-order', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
...
async function generatePlanSessions(params: {
  ...
  try {
    ...
    const endDate = params.finishingDate ? new Date(params.finishingDate) : new Date(Date.now() + 8 * 7 * 24 * 60 * 60 * 1000); // Default 8 weeks
...
function generateBasicSessions(params: {
  ...
  for (let week = 0; week < params.weeks; week++) {
    for (let sessionNum = 0; sessionNum < sessionsPerWeek; sessionNum++) {
      ...
      sessionDate.setDate(params.startDate.getDate() + (week * 7) + (sessionNum * 2)); // Every other day
