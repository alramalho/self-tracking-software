...
router.get('/', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    logger.info(`Retrieved ${metrics.length} metrics for user ${req.user!.id}`);
...
// Log a metric rating
router.post('/log-metric', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { metric_id, rating, date, description, skipped = false, description_skipped = false } = req.body;
    ...
    if (existingEntry) {
      ...
      if (description_skipped !== undefined) updateData.descriptionSkipped = description_skipped;
...
router.post('/skip-metric', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
...
router.get('/metric-entries', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    logger.info(`Retrieved ${metricEntries.length} metric entries for user ${req.user!.id}`);
...
router.delete('/metrics/:metricId', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (metric.userId !== req.user!.id) {
      return res.status(403).json({ error: 'Not authorized to delete this metric' });
    }
...
router.post('/log-todays-note', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { note } = req.body;
    ...
    if (todaysEntries.length === 0) {
      return res.status(404).json({ error: 'No metric entries found for today' });
    }
    ...
    logger.info(`Added note to ${updateResult.count} metric entries for user ${req.user!.id}`);
...
router.post('/skip-todays-note', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const today = new Date().toISOString().split('T')[0]; // YYYY-MM-DD format
    ...
    logger.info(`Marked ${updateResult.count} metric entries as description skipped for user ${req.user!.id}`);
