...
import { SuggestionBase, PlanGoalSuggestionData, PlanActivitySuggestionData, PlanTypeSuggestionData, PlanSessionsSuggestionData } from '@/types/suggestions';
  ...
  <div className="border-l-4 border-green-500 pl-3">
    ...
    <div className="flex flex-wrap gap-2">
      {activities.map((a, idx) => (
        <span key={idx} className="inline-flex items-center gap-1 bg-gray-100 px-2 py-1 rounded">
...
const PlanTypeSnippet: React.FC<{
  ...
  <div className="border-l-4 border-purple-500 pl-3">
    ...
    <p className="text-gray-900 capitalize">{planType.replace(/_/g, ' ')}</p>
  </div>
);
...
export const PlanBuildingContainer: React.FC<PlanBuildingContainerProps> = ({
  ...
  const goalSuggestion = useMemo(() => 
    suggestions.filter(s => s.type === "plan_goal").pop() as PlanGoalSuggestionData | undefined
  , [suggestions]);

  const activitiesSuggestion = useMemo(() => 
    suggestions.filter(s => s.type === "plan_activities").pop() as PlanActivitySuggestionData | undefined
  , [suggestions]);

  const planTypeSuggestion = useMemo(() => 
    suggestions.filter(s => s.type === "plan_type").pop() as PlanTypeSuggestionData | undefined
  , [suggestions]);

  const sessionsSuggestion = useMemo(() => 
    suggestions.filter(s => s.type === "plan_sessions").pop() as PlanSessionsSuggestionData | undefined
  , [suggestions]);
  ...
  return (
    <div className="flex flex-col gap-4 bg-white p-4 rounded-lg shadow-sm">
      ...
      {activitiesSuggestion && (
        <PlanActivitiesSnippet activities={activitiesSuggestion.data.activities} />
      )}
