...
async def get_metrics(user: User = Depends(is_clerk_user)):
    ...
    return metrics_gateway.get_all_metrics_by_user_id(user.id)
...
async def get_metric_entries(
    ...
    try:
        ...
        return metrics_gateway.get_all_metric_entries_by_user_id(user.id)
    except Exception as e:
        logger.error(f"Error getting metric entries: {str(e)}\n{traceback.format_exc()}")
