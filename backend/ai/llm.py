...
def get_model(model: str = LLM_MODEL):
    if 'lmstudio' in model:
        ...
        return GeminiModel(model_name=model, provider=GoogleVertexProvider(api_key=GEMINI_API_KEY))
    elif 'gpt' in model:
        return OpenAIModel(model_name=model, provider=OpenAIProvider(api_key=OPENAI_API_KEY))
    elif 'deepseek' in model:
        return OpenAIModel(model_name=model, provider=DeepSeekProvider(api_key=DEEPSEEK_API_KEY))
    elif 'mistral' in model:
        return MistralModel(model_name=model, provider=MistralProvider(api_key=MISTRAL_API_KEY))
    else:
